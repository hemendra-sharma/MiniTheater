...
public class StreamOverHttp {
    ...
    public StreamOverHttp(File f, String forceMimeType, long forceFileSize,
    ...
    private class HttpSession implements Runnable{
        ...
        private RandomAccessInputStream openRandomAccessInputStream (File file) throws IOException {
    ...
    private void copyStream(InputStream in, OutputStream out, byte[] tmpBuf, long maxSize) throws IOException{
        ...
        while(maxSize>0){
            ...
            while(in.available() < count){
                if(!buffering) {
                    streamListener.onBufferingStarted();
                    buffering = true;
                }
            }
            if(buffering) {
                streamListener.onBufferingStopped();
                buffering = false;
            }
    ...
    private void sendResponse(Socket socket, String status,
                                      String mimeType, Properties header,
                                      InputStream isInput, int sendCount,
                                      byte[] buf, String errMsg) {
        try{
    ...
    private void log (String msg) {
        if (debug) Log.d("StreamProxy", msg);
    }
}
